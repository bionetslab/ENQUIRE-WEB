...
def run(tag,to_py,thr=1,comb=4,A=2,K=3,ncores=6, wd="os.getcwd()", etype='all'):
	#
	execstr="./code/ENQUIRE.sh -p %s -t %s -i %s -r %s -c %s -a %s -k %s -e %s -j %s > stdout.txt 2> stderr.txt "
	#
